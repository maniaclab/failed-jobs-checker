...
def print_table(table):
    ...
    if len(rows) >= MAX_RESULTS:
        print(
            "NOTE: There may be more results than presented here. Please consider tuning MAX_RESULTS in check2.py or reducing the query interval"
        )
...
def get_all_failed(es_object, startdate, enddate, computingsite):
    body = {
        ...
        "query": {
            "bool": {
                "must": [{"match_phrase": {"jobstatus": "failed"}}],
                "should": [],
                "must_not": [],
                "filter": [
                    {
                        "bool": {
                            "should": [
                                {"term": {"computingsite": {"value": computingsite}}}
                            ]
                        }
                    },
                    {
...
def get_local_job_ids():
    ...
    ret = subprocess.run(
        job_command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
    if ret.returncode != 0:
        raise Exception("Subprocess did not exit cleanly: %s" % ret.stderr)
    job_ids = ret.stdout.decode().split()
    return job_ids
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--site",
        default="MWT2",
        help="Site, e.g. 'MWT2', 'AGLT2', 'SWT2_CPB', etc",
    )
    parser.add_argument(
        "-i", "--ids", nargs="+", help="space separated list of PanDA job IDs"
    )
    parser.add_argument(
        "-l",
        "--last",
        default=4,
        type=int,
        help="Maximum number of hours ago. Only relevant for 'site' mode",
    )
